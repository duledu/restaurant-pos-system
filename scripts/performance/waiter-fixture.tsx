...
window.fetch = async (input, options = {}) => {
  ...
  const entry = { url, method, start, duration: 0 }; bench.requests.push(entry);
  ...
  else if (url.startsWith('/api/pos/orders?tableId=')) {
    const id = new URL(url, location.origin).searchParams.get('tableId')!;
    result = { table: { id, label: `Table ${id}`, locationId: 'l' }, order: id === '24' ? null : orders.get(id) };
  }
  else if (url === '/api/pos/orders') {
    const { items: _items, table: _table, ...order } = orders.get(body.tableId)!;
    result = { order }; // Match the legacy POST's bare metadata response.
  }
  else {
...
function App() {
  ...
  return <Profiler id="waiter" onRender={(_, phase, actualDuration, baseDuration, startTime, commitTime) => {
    const usableOrder = Boolean(document.querySelector('.fixed.bottom-0'));
    bench.renders.push({ phase, actualDuration, baseDuration, startTime, commitTime, usableOrder, inspectedEmpty: !usableOrder && [...document.querySelectorAll('button')].some(b => b.textContent === 'Započni porudžbinu') });
  }}>
