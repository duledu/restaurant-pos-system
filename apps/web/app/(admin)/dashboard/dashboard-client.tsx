...
// ── Tipovi (ogledaju JSON odgovore analytics-service.ts — Faza 7) ────────
...
interface CurrentStatus { openTables: number; activeOrders: number; openShiftsCount: number; shiftsOnDuty: { shiftId: string; employeeName: string; role: string; openedAt: string }[]; }
interface SuspiciousSignal { category: string; severity: Severity; employeeId: string; employeeName: string; description: string; occurredAt: string; count?: number; value?: string; }
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [
        ...
        employeesNormalized, voids, discounts, payments, shifts, insightsRes, statusRes, attentionRes,
      ] = await Promise.all([
      ...
      setData({
        ...
        insights: insightsRes.insights, status: statusRes, signals: attentionRes.signals,
      });
    } catch (e) {
      setError(e instanceof Error ? e.message : "Greška pri učitavanju analitike");
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="mx-auto max-w-6xl">
      ...
      {loading || !data ? (
        ...
        <>
          {/* ── Restoran sada / Zahteva pažnju (Faza 5, nepromenjeno) ────── */}
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            <section>
              ...
              <Card className="p-5">
                <div className="mb-4 grid grid-cols-3 gap-3 text-center">
                  ...
                  <div><p className="text-2xl font-bold text-ink">{data.status.openShiftsCount}</p><p className="text-xs text-inkSoft">Otvorene smene</p></div>
                </div>
                  ...
                  <div className="space-y-2 border-t border-line pt-3">
                    ...
                    {data.status.shiftsOnDuty.map((s) => (
                      <div key={s.shiftId} className="flex items-center justify-between text-sm">
                        ...
                        <span className="text-xs text-inkSoft">od {new Date(s.openedAt).toLocaleTimeString("sr-RS", { hour: "2-digit", minute: "2-digit" })}</span>
                      </div>
                    ))}
                  </div>
                ) : (
            ...
            <section>
              <div className="mb-3 flex items-center justify-between">
                ...
                <Link href="/activity" className="text-xs font-medium text-gold-dark hover:underline">Sva aktivnost →</Link>
              </div>
              <Card className="p-5">
                  ...
                  <ul className="space-y-3">
                    {sortedSignals.slice(0, 6).map((s, i) => (
                      <li key={i} className="border-b border-line pb-3 last:border-0 last:pb-0">
                        ...
                        <p className="mt-0.5 text-xs text-inkSoft">{s.employeeName} · {new Date(s.occurredAt).toLocaleString("sr-RS")}</p>
                      </li>
                    ))}
                  </ul>
                )}
          ...
          <section className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-4">
            ...
            <KpiCard
              label="Neto prodaja"
              value={formatMoney(data.kpi.current.netSales, currency)}
              changePercent={data.kpi.previousAvailable && data.kpi.previous ? pctChange(data.kpi.current.netSales, data.kpi.previous.netSales) : undefined}
            />
            <KpiCard
              label="Porudžbine"
            ...
            />
            <KpiCard label="Gotovina" value={formatMoney(data.kpi.current.cashSales, currency)} />
            <KpiCard label="Kartica" value={formatMoney(data.kpi.current.cardSales, currency)} />
            <KpiCard label="Popusti" value={formatMoney(data.kpi.current.discountTotal, currency)} />
            <KpiCard label="Storna" value={formatMoney(data.kpi.current.voidTotal, currency)} />
          </section>
          ...
          {/* ── Uvidi (deterministički) ────────────────────────────────── */}
          {data.insights.length > 0 && (
            <section className="mt-6">
              <Card className="p-5">
                <h2 className="mb-3 text-xs font-semibold uppercase tracking-widest text-inkSoft">Uvidi</h2>
                <ul className="space-y-2">
                  {data.insights.map((insight, i) => (
                    <li key={i} className={`flex items-start gap-2 text-sm ${INSIGHT_COLOR[insight.tone]}`}>
          ...
          {/* ── Top / najslabije prodavani artikli ──────────────────────── */}
          <section className="mt-8">
            <div className="mb-3 flex items-center justify-between">
              <h2 className="text-xs font-semibold uppercase tracking-widest text-inkSoft">Top proizvodi</h2>
          ...
          <section className="mt-8">
            <div className="mb-3 flex items-center justify-between">
              <h2 className="text-xs font-semibold uppercase tracking-widest text-inkSoft">Prodaja po zaposlenom (TOP SALES BY EMPLOYEE)</h2>
              <div className="no-print flex gap-1 rounded-md bg-ink/[0.04] p-1">
